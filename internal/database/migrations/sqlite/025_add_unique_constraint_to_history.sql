...
INSERT OR IGNORE INTO import_history_new (
    ...
    virtual_path, category, completed_at
) SELECT 
    id, download_id, nzb_id, nzb_name, file_name, file_size, 
    virtual_path, category, completed_at
FROM import_history 
ORDER BY download_id DESC; -- Records with GUIDs come first
...
-- +goose Down
-- +goose StatementBegin
-- We don't strictly need to remove the UNIQUE constraint on down, but for completeness:
CREATE TABLE import_history_old (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    download_id TEXT DEFAULT NULL,
    nzb_id INTEGER,
    nzb_name TEXT NOT NULL,
...
INSERT INTO import_history_old SELECT * FROM import_history;
DROP TABLE import_history;
ALTER TABLE import_history_old RENAME TO import_history;
